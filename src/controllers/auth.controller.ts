 ...
 */
const setUserInfo = (req: UserRequest): Record<string, any> => {
  let user: any = {
    _id: req._id,
    name: req.name,
    email: req.email,
...
const saveUserAccessAndReturnToken = (req, user) =>
  new Promise(async (resolve, reject) => {
    try {
      const userAccess: any = new UserAccess({
        email: user.email,
        ip: utils.getIP(req),
...
const saveForgotPassword = async (req) =>
  new Promise((resolve, reject) => {
    const forgot: any = new ForgotPassword({
...
export const login = async (req: Request, res: Response): Promise<void> => {
  try {
    const { body } = req;
    const user: any = await findUser(body.email);
    await userIsBlocked(user);
    await checkLoginAttemptsAndBlockExpires(user);
    const isPasswordMatch: any = await auth.checkPassword(body.password, user);
    if (!isPasswordMatch) {
      utils.handleError(res, (await passwordsDoNotMatch(user)) as any);
    } else {
...
export const register = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!doesEmailExists) {
      const item: any = await registerUser(body);
...
export const forgotPassword = async (
  ...
  try {
    ...
    const item: any = await saveForgotPassword(req);
...
export const resetPassword = async (
  ...
  try {
    const data = matchedData(req);
    const hasForgotPassword: any = await findForgotPassword(data.id);
    const user = await findUserToResetPassword(hasForgotPassword.email);
    await updatePassword(data.password, user);
    const result = await markResetPasswordAsUsed(req, hasForgotPassword);
...
export const getRefreshToken = async (
  ...
  try {
    const tokenEncrypted = req.headers.authorization
      .replace('Bearer ', '')
      .trim();
    let userId: any = await getUserIdFromToken(tokenEncrypted);
    userId = await utils.isIDGood(userId);
    const user = await findUserById(userId);
    const token: any = await saveUserAccessAndReturnToken(req, user);
